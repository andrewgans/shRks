...
#plot
gom <- basemap( limits=c(-72, -66, 41, 45))
...
```{r Spatial Join Code}
# Assuming dat2 and dataq are your tibbles with coordinates and temperatures
...
```{r modeling, echo = F}
model <- lmer(julian_day ~ sst + (1 | species), data = result_tibble) #idk if we should be predicting individual count tho
Anova(model) 
```
...
If SST not a predictor --> maybe other factors (ocean acidification, prey migration, human activity, sea temperature but not surface etc.) that aren't directly correlated to sea SURFACE temperature
